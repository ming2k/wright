...
impl Builder {
    ...
    pub async fn build(
        ...
        if stages.is_empty() {
            if !tokio::fs::metadata(work_dir.join(".extracted")).await.is_ok() {
        ...
        let cpu_count = nproc_per_isolation.or(self.config.build.nproc_per_isolation).unwrap_or(total_cpus);
        ...
        if let Some(OutputConfig::Multi(ref parts)) = manifest.outputs {
            ...
            for (sub_name, sub_part) in parts {
                ...
                tokio::fs::create_dir_all(&sub_output_dir).await.map_err(|e| {
                    WrightError::BuildError(format!("failed to create sub-part directory {}: {}", sub_output_dir.display(), e))
                })?;
                let mut includes = Vec::new();
                if let Some(ref incs) = sub_part.include {
                    for pat in incs {
                        let re = regex::Regex::new(pat).map_err(|e| WrightError::BuildError(format!("invalid include regex '{}' for {}: {}", pat, sub_name, e)))?;
                        includes.push(re);
                    }
                }
                ...
                if let Some(ref excs) = sub_part.exclude {
                    for pat in excs {
                        let re = regex::Regex::new(pat).map_err(|e| WrightError::BuildError(format!("invalid exclude regex '{}' for {}: {}", pat, sub_name, e)))?;
                        excludes.push(re);
                    }
                }
                        ...
                        for (_sub_name, sub_dir, includes, excludes) in &sub_rules {
                            ...
                            if matched {
                                ...
                                if let Err(e) = tokio::fs::rename(&file_path, &dest_path).await {
                                    return Err(WrightError::BuildError(format!("failed to move {} to {}: {}", file_path.display(), dest_path.display(), e)));
                                }
                                break;
                            }
                        }
    ...
    pub async fn clean(&self, manifest: &PlanManifest) -> Result<()> {
        ...
        if tokio::fs::metadata(&build_root).await.is_ok() {
            tokio::fs::remove_dir_all(&build_root).await.map_err(|e| {
                WrightError::BuildError(format!("failed to clean build directory {}: {}", build_root.display(), e))
            })?;
    ...
    pub async fn verify(&self, manifest: &PlanManifest) -> Result<()> {
        ...
        for (i, source) in manifest.sources.entries.iter().enumerate() {
            ...
            let filename = http.r#as.clone().unwrap_or_else(|| source_cache_filename(&manifest.plan.name, &processed_url));
            ...
            if actual_hash != http.sha256 {
                return Err(WrightError::ValidationError(format!("SHA256 mismatch for {}:\n  expected: {}\n  actual:   {}", filename, http.sha256, actual_hash)));
            }
            ...
            match source {
                Source::Git(git) => {
                    ...
                    let cache_str = cache_path.to_str().ok_or_else(|| {
                        WrightError::BuildError(format!("git cache path contains non-UTF-8 characters: {}", cache_path.display()))
                    })?;
                    let repo = git2::Repository::clone(cache_str, &final_dest).map_err(|e| {
                        WrightError::BuildError(format!("local git clone failed: {}", e))
                    })?;
                    let (object, reference) = repo.revparse_ext(git_ref).or_else(|_| repo.revparse_ext(&format!("origin/{}", git_ref))).map_err(|e| {
                        WrightError::BuildError(format!("failed to resolve ref {}: {}", git_ref, e))
                    })?;
                    repo.checkout_tree(&object, None).map_err(|e| WrightError::BuildError(format!("git checkout failed: {}", e)))?;
                    match reference {
                        Some(gref) => {
                            let ref_name = gref.name().ok_or_else(|| WrightError::BuildError("git reference name is non-UTF-8".to_string()))?;
                ...
                Source::Http(http) => {
                    ...
                    if is_part_file(&filename) {
                        ...
                        compress::extract_part(&cache_path, &final_dest).map_err(|e| {
                            WrightError::BuildError(format!("failed to extract source {}: {}", filename, e))
                        })?;
                        ...
                        tokio::fs::copy(&cache_path, &dest).await.map_err(|e| {
                            WrightError::BuildError(format!("failed to copy non-archive source {} to work directory: {}", filename, e))
                        })?;
                    }
                }
                Source::Local(local) => {
                    ...
                    if is_part_file(&filename) {
                        ...
                        let pb = progress::new_source_spinner(&label, "extracting");
                        compress::extract_part(&cache_path, &final_dest).map_err(|e| {
                            WrightError::BuildError(format!("failed to extract local source {}: {}", filename, e))
                        })?;
                        progress::finish_source(&pb, &manifest.plan.name, &cache_path);
                    } else {
                        let dest = final_dest.join(&filename);
                        tokio::fs::copy(&cache_path, &dest).await.map_err(|e| {
                            WrightError::BuildError(format!("failed to copy local source {} to work directory: {}", filename, e))
                        })?;
                    }
                }
            }
    ...
    pub async fn update_hashes(&self, manifest: &PlanManifest, manifest_path: &Path) -> Result<()> {
        ...
        if !tokio::fs::metadata(cache_dir).await.is_ok() {
            tokio::fs::create_dir_all(cache_dir).await.map_err(WrightError::IoError)?;
        }
        for source in manifest.sources.entries.iter() {
            match source {
                Source::Http(http) => {
                    let processed_url = self.process_uri(&http.url, manifest);
                    let cache_filename = http.r#as.clone().unwrap_or_else(|| source_cache_filename(&manifest.plan.name, &processed_url));
                    let cache_path = cache_dir.join(&cache_filename);
                    if tokio::fs::metadata(&cache_path).await.is_ok() {
                        debug!("Using cached source: {}", cache_filename);
                    } else {
                        info!("Downloading {}...", processed_url);
                        download::download_file(&processed_url, &cache_path, self.config.network.download_timeout, &manifest.plan.name)?;
                    }
        ...
        let content = tokio::fs::read_to_string(manifest_path).await.map_err(WrightError::IoError)?;
        ...
        let new_content = if has_array_of_tables {
            ...
            while let Some(m) = sha256_re.find(&result[..]) {
                if hash_idx < new_hashes.len() {
                    let replacement = format!("{}\"{}\"", &result[m.start()..m.start() + result[m.start()..].find('"').unwrap()], new_hashes[hash_idx]);
            ...
            let hashes_str = new_hashes.iter().map(|h| format!("    \"{}\"", h)).collect::<Vec<_>>().join(",\n");
        ...
        tokio::fs::write(manifest_path, new_content).await.map_err(WrightError::IoError)?;
        Ok(())
    }

    async fn fetch_git_repo(&self, git_url: &str, git_ref: Option<&str>, dest: &Path, scope: &str) -> Result<String> {
        ...
        let repo = if !tokio::fs::metadata(dest).await.is_ok() {
            ...
            git2::Repository::init_bare(dest).map_err(|e| WrightError::BuildError(format!("git init failed: {}", e)))?
        } else {
            git2::Repository::open_bare(dest).map_err(|e| WrightError::BuildError(format!("git open failed: {}", e)))?
        };
        let mut remote = repo.remote_anonymous(git_url).map_err(|e| WrightError::BuildError(format!("git remote setup failed: {}", e)))?;
        ...
        callbacks.transfer_progress(move |stats| {
            ...
            progress::set_source_git_objects(&pb_clone, stats.received_objects() as u64, total, stats.received_bytes() as u64);
            true
        });
        let mut fetch_opts = git2::FetchOptions::new();
        fetch_opts.remote_callbacks(callbacks);
        fetch_opts.download_tags(git2::AutotagOption::All);
        let fetch_result = remote.fetch(&["+refs/heads/*:refs/heads/*", "+refs/tags/*:refs/tags/*"], Some(&mut fetch_opts), None);
        if fetch_result.is_err() {
            return Err(WrightError::BuildError(format!("git fetch failed: {}", fetch_result.unwrap_err())));
        }
        pb.set_message("indexing");
        progress::finish_source(&pb, scope, dest);
        let obj = repo.revparse_single(actual_ref).map_err(|e| WrightError::BuildError(format!("failed to resolve git ref '{}': {}", actual_ref, e)))?;
        Ok(obj.id().to_string())
    }
        ...
        for source in &manifest.sources.entries {
            match source {
                Source::Git(git) => {
                    ...
                    let commit_id = self.fetch_git_repo(&processed_url, git.r#ref.as_deref(), &dest, &manifest.plan.name).await?;
                ...
                Source::Http(http) => {
                    ...
                    if needs_download {
                        download::download_file(&processed_url, &dest, self.config.network.download_timeout, &manifest.plan.name)?;
                        if !skip_verify {
                ...
                Source::Local(local) => {
                    ...
                    tokio::fs::copy(&local_path, &dest).await.map_err(|e| {
                        WrightError::BuildError(format!("failed to copy local file {} to cache: {}", local_path.display(), e))
                    })?;
