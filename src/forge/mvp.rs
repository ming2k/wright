...
pub(crate) struct PlanGraph {
    ...
    pub(crate) rebuild_reasons: HashMap<String, crate::resolve::RebuildReason>,
...
pub(crate) fn collect_phase_deps(
    ...
    dep_domain: crate::resolve::DepDomain,
) -> Vec<String> {
    ...
    if dep_domain.contains(crate::resolve::DepDomain::BUILD) {
        raw_deps.extend(build.clone());
    }
    if dep_domain.contains(crate::resolve::DepDomain::RUNTIME) {
        raw_deps.extend(runtime.clone());
    }
    if dep_domain.contains(crate::resolve::DepDomain::LINK) {
        raw_deps.extend(link.clone());
    }
...
pub(crate) fn cycle_candidates_for(cycle: &[String], graph: &PlanGraph) -> Vec<CycleCandidate> {
    ...
    for part in cycle {
        ...
        let full_deps = collect_phase_deps(
            &manifest,
            &graph.part_to_plan,
            false,
            None,
            crate::resolve::DepDomain::ALL,
        );
        let mvp_deps = collect_phase_deps(
            &manifest,
            &graph.part_to_plan,
            true,
            None,
...
pub(crate) fn inject_bootstrap_passes(graph: &mut PlanGraph) -> Result<()> {
    ...
    for cycle in &cycles {
        ...
        let bootstrap_deps = collect_phase_deps(
            &mvp_manifest,
            &graph.part_to_plan,
            true,
            None,
