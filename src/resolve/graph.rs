...
pub(super) async fn expand_missing_dependencies(
    ...
    while let Some((name, depth)) = queue.pop_front() {
        ...
        if domain.contains(DepDomain::BUILD) {
            deps_to_check.extend(manifest.build_deps.iter().cloned());
        }
        if domain.contains(DepDomain::LINK) {
            deps_to_check.extend(manifest.link_deps.iter().cloned());
        }
        if domain.contains(DepDomain::RUNTIME) {
...
pub(super) fn build_dep_map(
    ...
    for (name, manifest) in &build_manifests {
        ...
        if dep_domain.contains(DepDomain::BUILD) {
            all_deps.extend(manifest.build_deps.iter());
        }
        if dep_domain.contains(DepDomain::RUNTIME) {
            all_deps.extend(manifest.runtime_deps.iter());
        }
        if dep_domain.contains(DepDomain::LINK) {
            all_deps.extend(manifest.link_deps.iter());
        }
        for dep_raw in &all_deps {
        ...
        if !checksum {
            deps = collect_phase_deps(manifest, &part_to_plan, is_mvp, Some(index), dep_domain);

            if is_mvp {
                let full_deps =
                    collect_phase_deps(manifest, &part_to_plan, false, Some(index), dep_domain);
                let mvp_deps =
                    collect_phase_deps(manifest, &part_to_plan, true, Some(index), dep_domain);
