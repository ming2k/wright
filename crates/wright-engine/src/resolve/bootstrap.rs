...
pub(crate) struct PlanGraph {
    ...
    pub(crate) rebuild_reasons: HashMap<String, super::RebuildReason>,
...
pub(crate) fn collect_phase_deps(
    ...
    if dep_domain.contains(super::DepDomain::BUILD) {
        raw_deps.extend(build.clone());
    }
    if dep_domain.contains(super::DepDomain::RUNTIME) {
        raw_deps.extend(runtime.clone());
    }
    if dep_domain.contains(super::DepDomain::LINK) {
