...
```bash
wright resolve <changed-part> --include-targets --dependents=all --depth=0 | wright build --force
```

Build, then install the resulting archives:

```bash
wright resolve <changed-part> --include-targets --dependents=all --depth=0 | wright build --force --print-archives | wright install
```

When you also want a deep upstream refresh, include dependency force-rebuild:

```bash
wright resolve <changed-part> --include-targets --deps=all --dependents=all --depth=0 | wright build --force --print-archives | wright install
```
...
3. Run conservative rebuild with `wright resolve <part> --include-targets --dependents | wright build --force --print-archives | wright install` first; switch to `--dependents=all --depth=0` only when the change is broader than link-ABI impact.
