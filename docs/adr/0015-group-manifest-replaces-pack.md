# ADR-0015: Group Manifest Replaces Pack Format
...
1. **No collection abstraction.**  Users had to list every plan name on the
   ...
   There was no way to say "install the `@core` group" and have Wright resolve
...
2. **Introduce the group manifest** (`group.toml`) — a pure declaration that
   names plans, assumed externals, and optional post-install configuration.
3. **Extend `wright launch`** to accept either `--group <file>` or `--plans <dir>`
   (with optional `@group` references), drive the full
...
Unlike `pack.toml`, a group does **not** reference pre-built archives or carry
an overlay tree.  It is a build recipe, not a binary bundle.

### `wright launch` changes

```bash
# From an explicit group file
...
# From a plans directory using a group reference
...
`<root>/var/lib/wright/plans/` and any referenced group manifests into
`<root>/var/lib/wright/groups/`, then writes a minimal
...
- One manifest = one bootstrappable system.  No need to maintain pre-built
  archive bundles; the group stays current as plans evolve.
...
- `@group` references let users compose systems without enumerating every plan.
