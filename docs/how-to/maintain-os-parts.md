...
```bash
wright resolve <changed-part> --rdeps=all --depth=0 | wright build --force --package --print-parts | wright install
```

When you also want a deep dependency refresh, include dependency force-rebuild:

```bash
wright resolve <changed-part> --deps=all --rdeps=all --depth=0 | wright build --force --package --print-parts | wright install
```
...
3. Run conservative rebuild with `wright resolve <part> --rdeps | wright build --force --package --print-parts | wright install` first; switch to `--rdeps=all --depth=0` only when the change is broader than link-ABI impact.
