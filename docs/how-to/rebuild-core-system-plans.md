# How to Rebuild Core System Plans

When you need to update the core toolchain (binutils, gcc, glibc, linux, or
linux-api-headers) — whether for a security patch or a major version upgrade —
you must rebuild the affected plans in a specific order. Rebuilding out of
order produces binaries that link against the old runtime.

## Required rebuild order

Run `wright install` for each plan in this exact sequence. Do not skip steps.
...
wright install $(wright list | grep -v -E '^(binutils|gcc|glibc|linux-api-headers|linux)$')
```

Wright's `install` command will automatically remove the old parts produced by
