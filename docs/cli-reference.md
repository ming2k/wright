...
| `--install` (`-i`) | Install each completed package directly to host `/` between dependency waves. Equivalent to running `wright install` after each build wave. User-specified targets get `build` origin; auto-resolved dependencies get `dependency` origin. |
