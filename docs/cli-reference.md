...
| `--install` (`-i`) | Build against a session-local overlay sysroot, stage completed packages into that session root between dependency waves, defer package install/upgrade hooks during staging, then commit them to host `/` and run hooks there after all tasks succeed. User-specified targets get `build` origin; auto-resolved dependencies get `dependency` origin. |
...
# Resume after a partial failure (auto-detect session)
wbuild resolve gtk4 --self --dependents | wbuild run --resume -i

# Resume with an explicit session hash
wbuild resolve gtk4 --self --dependents | wbuild run --resume a1b2c3... -i
