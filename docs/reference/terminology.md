...
| **Target identifier** | The universal plan/output grammar accepted by commands that act on deployed state: `plan` or `plan:*` addresses every deployed output of a plan, `output` or `plan:output` addresses one output. An ambiguous bare name is rejected with the absolute forms to use. See [ADR-0034](../adr/0034-plan-output-namespaces-and-part-layout.md). |
...
| **Merged base** | The `base/` directory under a build root: a reflink/hard-link union of the extracted source and all completed stage layers, used to populate the next stage's `target/` working tree. Rebuilt from the surviving layers whenever `.base_manifest` disagrees with the checkpoints. See [ADR-0037](../adr/0037-real-directory-stage-trees.md). |
