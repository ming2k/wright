...
- **Git sources gain a `submodules = true` flag for projects that vendor git submodules.** Declared on a `[[sources]]` git entry, it switches fetching to a full clone followed by `git submodule update --init --recursive`, and the result is snapshotted into the source cache with `.git` excluded and the flag mixed into the snapshot cache key, keeping builds reproducible and offline-capable.

### Changed
- **The `Cascading` reports in `wright remove` and `wright upgrade` render as aligned multi-line blocks.** The verb header carries the counts and each per-trigger row aligns under the message column via the continuation indent the `Timing` report uses, replacing single long comma-joined lines.
- **The native sandbox bind-mounts the host's `/opt` read-only when it exists and the new root lacks one**, mirroring the `/usr` bind, so plans relying on toolchain payloads under `/opt` build without extra configuration.
...
- **New `general.ledger_dir` config key** (default `/var/lib/wright/ledger`) locating the per-plan audit ledger. A database redirected via `--root`/`--db` keeps its ledger beside itself (`<db dir>/ledger`), mirroring the lock/journal derivation, so target-root audits never mix with the host's.

### Changed
- **Plan-source snapshots moved out of the database into ledger files (ADR-0041, amends ADR-0033).** Deploy/upgrade registration now appends `<ledger_dir>/<plan>/snapshots/<timestamp>-<checksum>.toml` (written only when the checksum changes) instead of inserting into `plan_snapshots`; `wright doctor` drift diffs and `wright plan` read the files, with unchanged behavior. On first open, any existing `plan_snapshots` rows are exported to files — rows whose plan no longer exists land in `_detached/` — and migration V20 then drops the table; a failed export aborts the open rather than lose data. `.PLANSRC` archive members are unaffected.
- **Durations of a minute or more now render as `2m 4s` / `2m 4.6s` instead of `2m04s`.** The shared `util::timing::format_duration` presentation rule is now: under one second → whole milliseconds (`96ms`); under one minute → seconds with one decimal (`1.1s`); one minute or more → minutes plus seconds, keeping the tenths digit only when it is nonzero (`2m 4s`, `2m 14.6s`). This affects every human-facing duration: the `Finished … in …` lines, the end-of-run `Timing` report, and the `lint` / `check` / `doctor` / `remove` summaries. Structured log fields (`total_secs`, `elapsed_secs`) are unchanged.
- **Failure output is now one contiguous block: a single failing task no longer prints its `error:` line twice, and the step-timing table closes the terminal failure report instead of splitting it.** Two refinements to the ADR-0039 failure reporting. First, the immediate `error: task '<name>' failed: <cause>` notice now fires only while batch siblings are still running — the failure that empties a batch is carried by the terminal failure report alone, so single-task failures (the common case) read as one `error:` headline plus its `Caused by:` list. Second, on failure the end-of-run step-timing report is deferred and appended after the `See … for the full trace` hint rather than rendered mid-error; its header no longer repeats a `(failed)` marker (the failed step row keeps its own), and the structured `workflow.timing` event still lands in the daily log file. Success-path output is unchanged.
- **`-c`/`--clean` on `build` and `install` is renamed to `-c`/`--fresh`, and `upgrade` and `launch` gain the same flag (ADR-0040).** The flag wipes the forge workspace before building so the forge starts from scratch; the old spelling read as if deletion were the command's purpose and collided with the standalone `wright clean` command. On `upgrade` and `launch`, `--force` previously implied the wipe; `--fresh` is now the independent spelling and `--force` still implies it. `--clean` keeps working as a hidden alias on all four commands for one release.
- **`wright prune` is merged into `wright clean` and deprecated (ADR-0040).** `clean` now owns all disk reclamation: build workspaces by default, `--archives` (renamed from `--parts`, which read as deleting deployed parts) for every built archive of the named plans, `--stale` for superseded archive versions only, `--logs` for command logs, and `-n`/`--dry-run` to preview the exact removal set. `wright prune` remains as a hidden alias for `wright clean --stale` for one release; unlike the canonical spelling it stays a dry run without `--apply`, matching its historical default. Migration: `prune --apply` → `clean --stale`; note the canonical spelling executes immediately, preview with `-n` first.
...
### Changed
- **Console output is BrokenPipe-safe by construction.** All stdout/stderr printing goes through new `outln!`/`out!`/`errln!` macros (`util::output`): a closed stdout reader (`wright list | head`) exits quietly with code 0 — previously 141 via SIGPIPE death, now the ripgrep/bat convention — and stderr diagnostics are best-effort and never panic. Raw `println!`/`eprintln!` in CLI code are denied workspace-wide via clippy (`print_stdout`/`print_stderr`) so the policy cannot silently regress.
...
### Changed
- **`build --rebuild` renamed to `build --force`** — the `-R`/`--rebuild` flag is now `-f`/`--force`, consistent with `install --force`, `launch --force`, and other subcommands. The short flag changed from `-R` to `-f`.
